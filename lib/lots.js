...
function searchTickets(directory, exclude, todos, cb) {
  // var cmd = 'grep -H -n -R -I "\\!" * ' + exclude + ' | grep "#" | grep "\\^[0-9]"'; // makes priority required
  var cmd = 'grep -H -n -R -I "\\!" * ' + exclude + ' | grep "#"';
  var opts = {
    cwd: directory,
    maxBuffer: 2000 * 1024
  };
  exec(cmd, opts, function(err, stdout, stderr) {
    ...
    if (todos) {
      exec('grep -H -n -R -I TODO * ' + exclude, opts, function(err, stdout, stderr) {
        if (err) {
          return cb('error searching for tickets ' + err, null);
        }
        if (stderr) {
