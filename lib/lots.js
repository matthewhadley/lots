/*
eslint standard/no-callback-literal: 0, node/no-deprecated-api: 0
*/
...
function searchTickets (opts, cb) {
  let patterns = '';

  if (opts.glob) {
    opts.glob.forEach(function (glob) {
      patterns += ' -g "' + glob + '"';
    });
  }

  var cmd = {
    // tickets: 'rg ' + opts.targets + ' -o -H -n -e "(^!| !).*#.*" | rg -e "\\^[0-9]"' + patterns, // makes priority required
    tickets: 'rg ' + opts.targets + ' -o -H -n -e "(^!| !).*#.*"' + patterns,
    todos: 'rg ' + opts.targets + ' -H -n TODO | rg TODO' + patterns
  };
...
module.exports = function LOTS (opts) {
  ...
  return {
    ...
    generate: function generate (cb) {
      searchTickets(opts, function (err, data) {
        if (err) {
          return cb(err);
        }
