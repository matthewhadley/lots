...
function searchTickets (directory, exclude, todos, cb) {
  var cmd = {};
  if (process.env.LOTS_RG === '1') {
    // !doesn't honor excludes #bug
    // ripgrep commands
    cmd = {
      // tickets: 'rg -o -H -n -e "(^!| !).*#.*" | rg -e "\\^[0-9]"', // makes priority required
      tickets: 'rg -o -H -n -e "(^!| !).*#.*"',
    ...
    // grep commands
    cmd = {
      // tickets: 'grep -o -H -n -R -I -E "(^!| !).*#.*" * ' + exclude + '| grep "\\^[0-9]"', // makes priority required
      tickets: 'grep -o -H -n -R -I -E "(^!| !).*#.*" * ' + exclude,
